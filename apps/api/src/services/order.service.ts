...
async function sendHourCoffeeReadyEmailIfNeeded(order: any, source: string) {
  ...
  if (!customerEmail) {
    console.warn('[hour-coffee-email] READY skipped (missing customerEmail)', { source, orderId });
    return;
  }

  if (!isValidEmail(customerEmail)) {
    console.warn('[hour-coffee-email] READY skipped (invalid customerEmail)', { source, orderId, customerEmail });
    return;
  }
  ...
  if (now - lastSentAt < READY_EMAIL_DEDUPE_TTL_MS) {
    console.log('[hour-coffee-email] READY duplicate-send skipped', { source, orderId, customerEmail });
    return;
  }
  ...
  try {
    ...
    if (booth?.name && String(booth.name).trim() !== '') boothName = String(booth.name).trim();
  } catch (e: any) {
